...
type (
	...
	HDWalletExport struct {
		...
		Addresses         []*HDAddressExport `json:",omitempty"`
		multiAddress
	}
	multiAddress struct {
...
const (
	SignatureSecp256k1 HDSignature = iota
	SignatureEd25519
	SignatureSr25519
	SignatureBls
)

func GenPrivKeyFromSecret(seed []byte, c HDSignature) (interface{}, error) {
...
func GetPublicKeyFromSeed(seed []byte, c HDSignature, compressed bool) ([]byte, error) {
	if c == SignatureEd25519 {
...
func (p *HDWallet) SetPath(path string) error {
...
func (p *HDWallet) SetIterations(iterations uint) error {
...
func (p *HDWallet) SetUseRawEntropy(e bool) error {
	p.useRawEntropy = e
	err := p.parseMnemonic()
	if err != nil {
		return err
...
func (p *HDWallet) parseMnemonic() error {
	// substrate / polkadot
...
func (p *HDWallet) ExportRootAddress() (*HDWalletExport, error) {
	pwe := new(HDWalletExport)
...
func (p *HDWallet) ExportHDAddresses(count int) (*HDWalletExport, error) {
	pwe := new(HDWalletExport)
	pwe.Mnemonic = p.Mnemonic
	pwe.Passphrase = p.Passphrase // ???
	pwe.Seed = hex.EncodeToString(p.rawSeed)
	...
	pwe.Addresses = make([]*HDAddressExport, 0)
...
func (p *HDWallet) GetKey() (*bip32.Key, error) {
...
func (p *HDWallet) GetKeyForPath(inputPath string) (*bip32.Key, error) {
...
func (p *HDWallet) parseDerivationPath() ([]uint32, error) {
